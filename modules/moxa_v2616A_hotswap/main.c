...
MODULE_AUTHOR("lock.lin@moxa.com");
MODULE_LICENSE("GPL");

#ifdef V2416A
MODULE_DESCRIPTION("MOXA V2416A hotswap driver");
#else
MODULE_DESCRIPTION("MOXA V2616A hotswap driver");
#endif
...
static int hotswap_led_control(int led_num, int on) {
	...
	if (on == 1) {  /* change status to 1, because high activate */
...
static long hotswap_ioctl(struct file *file, unsigned int cmd, unsigned long arg) 
{
	...
	switch (cmd) {
		...
		case IOCTL_CHECK_DISK_PLUGGED :
			...
			if (p.disk_num == 1) {
				p.val = (ioread32(vaddr + 0x220) == 0x7f) ? 0: 1;/* 0x7f for un-plug */
			}
			else if (p.disk_num == 2) {
				p.val = (ioread32(vaddr + 0x2A0) == 0x7f) ? 0: 1;/* 0x7f for un-plug */
			}
			else 
				return -EINVAL;
			if (copy_to_user(addr, &p, sizeof(mxhtsp_param))) 
...
#endif
{
	...
	mod_timer(&timer, jiffies+100*HZ/1000);
 	...
 	pp("disk1 r1=%x r2=%x disk2 r1=%x r2=%x\n", ioread32(vaddr+0x219), ioread32(vaddr+0x2a0), 
			ioread32(vaddr+0x299), ioread32(vaddr+0x220));
			//ioread32(vaddr+0x219), ioread32(vaddr+0x220));
}
...
static int __init hotswap_init_module (void) 
{
	...
	printk("Initializing MOXA hotswap module, %s\n", DRV_VERSION);

	/* get base address for ACHI controller */
	//dev = pci_get_device(PCI_VENDOR_ID_INTEL, 0x1c09, NULL);
	...
	*/
	...
	/* initial */
	d1.status = ioread32(vaddr + 0x219);
	d1.idle_cnt = 0;
	d2.status = ioread32(vaddr + 0x299);
	d2.idle_cnt = 0;
