...
#define	MXUPCIE_VERSION	"1.15.21"
...
int mx_init(void)
{
	...
	for(m = 0;m < MXUPCIE_BOARDS; m++)
	{
		...
		{
			int port_idx =  (m * MXUPCIE_PORTS_PER_BOARD) + i;
					 ...
					 int *status)
{
...
#if (LINUX_VERSION_CODE >= VERSION_CODE(3,9,0))
	recv_room = tty_buffer_request_room(tty->port,MX_RX_FIFO_SIZE);
#else
	...
	if(recv_room){
		...
		if(gdl){
#if (LINUX_VERSION_CODE >= VERSION_CODE(3,9,0))
			tty_insert_flip_string(tty->port, info->base + MOXA_PUART_MEMRBR, gdl);
#else
...
static int mx_set_interface(struct mxupcie_struct *info, unsigned char val)
{
	...
	switch(val){
		...
		case MOXA_UIR_RS485_2W:
			switch(info->board_type){
/* Jared, 2014-01-28, For V2616A, The VENDOR ID/DEVICE ID, 1393:1024, support RS232/RS422/RS4852W/RS4854W modes. We modify to skip the board type checking for V2616A to support the mode control.
*/
#ifndef V2616A
