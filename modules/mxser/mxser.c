...
#if (LINUX_VERSION_CODE >= 131394 && LINUX_VERSION_CODE < VERSION_CODE(2,6,26)) /* Linux 2.1.66 */
...
// (above) added by James.
	...
	for(m = 0;m < MXSER_BOARDS; m++)
	{
		...
		{
			int port_idx =  (m * MXSER_PORTS_PER_BOARD) + i;
					 ...
					 int *status)
{
	...
	int			max = 256;
	unsigned long 		flags;
#if 1 // Jared, low_latency use the work queue to push into tty fix the scheduling while atomic issue in interrupt context
...
intr_old:
	...
	do {
		...
		if ( *status & UART_LSR_SPECIAL ) {
		    ...
		    flags = 0;
#if (LINUX_VERSION_CODE >= VERSION_CODE(3,9,0))
		tty_insert_flip_char(tty->port, ch, flags);
#else
...
end_intr:	// add by Victor Yu. 09-02-2002
...
#if 1 // Jared, low_latency use the work queue to push the received data into tty buffer which fixes the scheduling while atomic issue in interrupt context
	spin_unlock(&info->slock);
	if( in_interrupt() ) {
#if (LINUX_VERSION_CODE >= VERSION_CODE(3,9,0))
		low_latency_backup = tty->port->low_latency;
	...
	if( in_interrupt() ) {
#if (LINUX_VERSION_CODE >= VERSION_CODE(3,9,0))
		tty->port->low_latency = low_latency_backup;
#else
...
#if (LINUX_VERSION_CODE >= VERSION_CODE(3,9,0))
	    info->tty->port->low_latency = (info->flags & ASYNC_LOW_LATENCY) ? 1 : 0;
#elif (LINUX_VERSION_CODE >= VERSION_CODE(2,1,0))
