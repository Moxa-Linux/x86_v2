...
int main(int argc, char *argv[]) {
	...
	if ( argc > 2 ) {
usage:
		printf("Reset the mini-PCIE module utility\n");
		printf("Usage: %s\n");
		exit(0);
	}
	...
	while ((c = getopt(argc, argv, optstring)) != -1)
		...
		case 'd':
			...
			break;
#if 1	/* Jared, 02-10-2014, The hardware reset bit not work.
	   We use the disable bit to reset the mini-PCIE module.
	*/
		case 'r':
			bPCIE_reset=1; 
			break;
	...
	if ( bPCIE_Wireless_disable == 1 ) {
		printf("Turn off the mini PCIe slot %d\n", slot.num);
		slot.value=0;	// Disable the Wireless module
		ret = ioctl(fd, IOCTL_PCIE_WDISABLE, &slot);
		sleep(1);
		printf("Turn on the mini PCIe slot %d\n", slot.num);
		slot.value=1;	// Enable the Wireless module
		ret = ioctl(fd, IOCTL_PCIE_WDISABLE, &slot);
	}
#if 1	/* Jared, 02-10-2014, The hardware reset bit not work.
	   We use the disable bit to reset the mini-PCIE module.
